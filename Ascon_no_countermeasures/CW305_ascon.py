...
class CW305_ascon(CW305):
    ...
    def __init__(self):
        ...
        self.default_verilog_defines = 'cw305_ascon_defines.v'
        self.default_verilog_defines_full_path = self.default_verilog_defines
        self.registers = 39 # number of registers we expect to find
    ...
    def set_key(self, data=b''):
        ...
        self.fpga_write(self.REG_CRYPT_DIN0, list(data[  0: l]))
    ...
    def set_nonce(self, data=b''):
        ...
        if len(data) < l:
            data += b'\x00' * (l - len(data))
        self.fpga_write(self.REG_CRYPT_DIN1, list(data[  0: l]))
    ...
    def set_ad(self, data=b''):
        ...
        if len(data) < l:
            data += b'\x00' * (l - len(data))
        self.fpga_write(self.REG_CRYPT_DIN2, list(data[  0: l]))
    ...
    def set_pt(self, data=b''):
        ...
        if len(data) < l:
            data += b'\x00' * (l - len(data))
        self.fpga_write(self.REG_CRYPT_DIN3, list(data[  0: l]))
    ...
    def get_result(self):
        l = 16
        ct  = bytes(self.fpga_read(self.REG_CRYPT_DOUT3, l))
        mac = bytes(self.fpga_read(self.REG_CRYPT_DOUT4, l))
        return ct, mac
    ...
    def run_operation(self, key=b'', nonce=b'', ad=b'', pt=b'', acq=False):
    ...
    def capture_trace(self, scope, key=b'', nonce=b'', ad=b'', pt=b'', acq=True):
        ...
        ct, tag = self.get_result()
        data = key + nonce + ad + pt + ct + tag
