...
import datetime
from CW305_ascon_TI import CW305_ascon as cw1 # Import of the chipwhisperer class
...
def acqTrace(project_name, NS=10, ttest=False):
    ...
    # The inputs are the secrets, they are later shared inside the HW implementation
...
fpga_id = None
platform = 'ss2_a35'
target = cw.target(scope, cw1, force=True, platform=platform, fpga_id=fpga_id, bsfile='./bitstream_husky_ascon_TI.bit')

scope.clock.clkgen_freq = 7.37e6
scope.io.hs2 = 'clkgen'
scope.clock.clkgen_src = 'system'
scope.clock.adc_mul = 4 #8 the multiplier between the ADC (analog-to-digital converter) clock and the main clock source (how many samples per clock cycle)
